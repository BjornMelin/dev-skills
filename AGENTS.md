...
- Local plugins: `node plugins/web-motion/scripts/validate-atomic-skills.mjs`, `node plugins/native-motion/scripts/validate-atomic-skills.mjs`, `node --check tools/skill/overhaul_motion_plugins.mjs`, and `rg --files -g '*.json' plugins | xargs -r jq empty`
