...
- A schema handed to `codex exec --output-schema` must be valid for **strict** structured outputs, not merely valid JSON Schema: every key in `properties` must also appear in `required`, and every object must set `additionalProperties: false`. Express an optional field as required-and-nullable (`"type": ["string", "null"]`). Omitting a key from `required` still parses and still passes `jq empty`, then fails at request time with a 400. `tools/skill/check_output_schemas.py` catches it in CI.
