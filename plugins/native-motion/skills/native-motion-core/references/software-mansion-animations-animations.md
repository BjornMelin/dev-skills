...
For new Reanimated 4 work, use `scheduleOnRN` instead of `runOnJS` to call
JS-thread functions from a worklet. `runOnJS` is deprecated and re-exported for
compatibility, so check the installed versions and migration scope before
treating existing maintenance code as broken. Arguments are passed directly,
not curried:

```tsx
// Legacy/deprecated compatibility API
runOnJS(setCount)(newCount);

// New Reanimated 4 work
