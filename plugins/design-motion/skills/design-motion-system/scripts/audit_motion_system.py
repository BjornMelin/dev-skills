...
IGNORE = {
    "node_modules", ".git", "dist", "build", ".next", ".expo", "coverage",
    "target", "out", ".turbo", ".cache", "vendor", ".venv", "__pycache__",
}
...
def audit_file(root: Path, p: Path):
    ...
    if len(durations) >= 3:
        findings.append({"severity": "medium", "type": "hardcoded-durations", "detail": sorted(set(durations))[:12]})
    if len(easings) >= 3:
    ...
    if "react-native-gesture-handler" in text and "velocity" not in text:
        findings.append({"severity": "medium", "type": "gesture-no-velocity", "detail": "Gesture code appears to omit velocity-aware release."})
    if ("prefers-reduced-motion" not in text and "useReducedMotion" not in text and "ReducedMotion" not in text and
        any(w in text for w in ["useFrame", "withSpring", "withTiming", "withRepeat", "parallax", "ScrollControls"])):
        findings.append({"severity": "medium", "type": "missing-reduced-motion-signal", "detail": "Motion code has no local reduced-motion signal."})
    return {"file": rel, "findings": findings} if findings else None


def main():
    ap = argparse.ArgumentParser(description="Static audit for motion-system risks.")
    ...
    for p in iter_files(root, args.limit):
        ...
        if item:
            results.append(item)
    summary = {"files_with_findings": len(results), "findings": sum(len(r["findings"]) for r in results)}
    print(json.dumps({"root": str(root), "summary": summary, "results": results}, indent=2 if args.pretty else None))
