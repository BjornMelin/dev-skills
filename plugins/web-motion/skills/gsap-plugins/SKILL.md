...
const helper = MotionPathHelper.create(".dot", {
  path: "#path",
  end: 0.5
});
...
- `references/official-source.md` - Official GreenSock plugin skill source. Use this to verify upstream plugin coverage and license.
- `references/plugin-availability.md` - Plugin import and availability matrix. Use this before writing imports, registration, or premium-plugin examples.
- `references/plugin-lifecycle.md` - Plugin setup, cleanup, and accessibility review. Use this for Flip, Draggable, Observer, SVG, and text plugin implementation review.
- `references/premium-plugin-license-boundaries.md` - Plugin availability and license boundaries. Read before generating imports or examples for SplitText, MorphSVG, DrawSVG, Club plugins, or any plugin absent from local dependencies.
- `references/plugin-specific-test-fixtures.md` - Plugin-specific fixtures and verification. Read when validating Flip, Draggable, Observer, MotionPath, ScrollTo, SVG, or text plugin behavior.
- `references/docs-plugin-current-notes.md` - Copied source excerpt. Load only when exact upstream wording or API detail is needed.
...
- `assets/templates/gsap-plugins-audit-report.md` - GSAP audit response template.
- `assets/templates/gsap-plugins-review-checklist.md` - GSAP manual review checklist.
- `assets/examples/gsap-plugins-starter.js` - Minimal starter fixture/example.
...
node scripts/audit.mjs scan --root . --format json --output gsap-plugins-audit.json
```
