...
- Broad multi-crate architecture routes to `rust-mega-eng` only when the user
  explicitly asks for `rust-mega-eng` or `$rust-mega-eng`.
...
- Keep `rust-mega-eng` explicit-only in both `SKILL.md` frontmatter and
  `agents/openai.yaml`.
