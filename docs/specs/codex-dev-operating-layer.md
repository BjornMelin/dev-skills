...
`pr-agent-sources/<timestamp>/`, records each source through the existing
`pr record` normalizers, and writes `pr-agent-state.json` with explicit
...
silently treating stale or partial evidence as clean. This command has no
hosted-write mode; future hosted-action and TUI lanes should consume the typed
report instead of rediscovering PR state ad hoc.
