...
Mutates and returns the same array in random order. Shuffle a copy when the original array is state, props, or shared data.
...
Wraps a numeric function and appends a unit to its returned value. Use when a reusable helper should produce CSS values or tween end values.
...
- `references/official-source.md` - Official GreenSock utils skill source. Use this to verify copied upstream helper guidance.
- `references/numeric-mapping.md` - Numeric mapping and snapping reference. Use this for clamp, mapRange, normalize, interpolate, snap, wrap, and wrapYoyo.
- `references/selectors-and-units.md` - Selector scoping and unit helper notes. Use this for toArray, selector, unitize, getUnit, and function-based values.
- `references/function-composition-recipes.md` - gsap.utils function composition recipes. Read when clamp, mapRange, normalize, interpolate, snap, pipe, unitize, wrap, or selector utilities feed animation values.
- `references/random-determinism-testing.md` - Randomness, wrapping, and deterministic testing. Read when `random`, `shuffle`, `wrap`, `wrapYoyo`, or function-based values affect visual output.
...
- `assets/templates/gsap-utils-audit-report.md` - GSAP audit response template.
- `assets/templates/gsap-utils-review-checklist.md` - GSAP manual review checklist.
- `assets/examples/gsap-utils-starter.js` - Minimal starter fixture/example.
...
node scripts/audit.mjs scan --root . --format json --output gsap-utils-audit.json
```
