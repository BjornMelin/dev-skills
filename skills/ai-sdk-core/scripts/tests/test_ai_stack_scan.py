...
class AiStackScanTests(unittest.TestCase):
    def test_default_output_redacts_root_and_source_evidence(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            (root / "package.json").write_text('{"dependencies":{"zod":"^3.22.0"}}', encoding="utf-8")
        ...
        self.assertTrue(all("evidence" not in signal for signal in data["signals"]))
    ...
    def test_symlinked_package_json_outside_root_is_ignored(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            root.mkdir()
            outside.mkdir()
            (outside / "package.json").write_text('{"dependencies":{"zod":"^3.22.0"}}', encoding="utf-8")
    ...
    def test_capped_traversal_is_sorted(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            (root / "b").mkdir()
            (root / "a").mkdir()
            source = "import { useChat } from '@ai-sdk/react';\nmessage.content\n"
        ...
        message_signals = [
            signal for signal in data["signals"] if signal["id"] == "ai_sdk_ui_message_content"
        ]
    ...
    def test_supabase_service_role_public_env_is_redacted(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            data = run_scan(root, "--family", "supabase-ts", "--include-evidence")

        self.assertEqual(data["signals"][0]["id"], "supabase_service_role_public_env")
