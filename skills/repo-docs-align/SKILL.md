...
Bundled resources:
- `scripts/new_repo_docs_align_artifact.py` - scaffold hidden working artifacts under `.agents/repo-docs-align/YYYY-MM/MM-DD/NN/`
...
- `.agents/repo-docs-align/YYYY-MM/MM-DD/NN/`
...
- ensure the repo ignores `.agents/` or, at minimum, `.agents/repo-docs-align/`
- keep canonical docs, ledgers, specs, ADRs, runbooks, and active execution catalogs in their true authority surfaces, not in `.agents/repo-docs-align/`
...
Run that command from the `repo-docs-align` skill directory so the script can resolve its bundled templates correctly.
