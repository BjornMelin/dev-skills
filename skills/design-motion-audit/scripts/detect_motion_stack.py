...
KEYWORDS = {
    ...
    "shader_files": ["ShaderMaterial", "gl_FragColor", "fragmentShader", "vertexShader", "uniform"],
...
def scan(root: Path, limit: int):
    ...
    packages = {name: any(pkg in deps for pkg in candidates) for name, candidates in PACKAGE_KEYS.items()}
    ...
    for p in iter_files(root, limit):
        ...
        # .json is used for asset/dep detection above, never for keyword scanning
        # (bare "motion"/"duration" in package.json / fixtures caused false positives).
    ...
    return {"root": str(root), "packages": packages, "matches": matches, "assets": assets}


def main():
    ap = argparse.ArgumentParser(description="Detect 3D, motion, and native animation stack signals in a repo.")
